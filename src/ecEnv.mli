...
module Auto : sig
  val dname  : symbol
  (* FIXME section: why add1 does not take import ? *)
  val add1   : local:is_local -> level:int -> ?base:symbol -> path -> env -> env
  val add    : ?import:import -> local:is_local -> level:int -> ?base:symbol -> path list -> env -> env
