...
let rec replay_tyd (ove : _ ovrenv) (subst, ops, proofs, scope) import (x, otyd) =
  ...
  | None ->
      ...
      (subst, ops, proofs, ove.ovre_hooks.hadd_item scope import (Th_type (x, otyd)))
  ...
  | Some { pl_desc = (tydov, mode) } -> begin
      ...
      let scope =
        match mode with
        | `Alias ->
          ove.ovre_hooks.hadd_item scope import (Th_type (x, newtyd))

        | `Inline `Keep ->
            ove.ovre_hooks.hadd_item scope EcTheory.noimport (Th_type (x, newtyd))
...
and replay_opd (ove : _ ovrenv) (subst, ops, proofs, scope) import (x, oopd) =
  ...
  | Some { pl_desc = (opov, opmode); pl_loc = loc; } ->
      ...
      let scope =
        match opmode with
        | `Alias ->
            ove.ovre_hooks.hadd_item scope import (Th_operator (x, newop))

        | `Inline `Keep ->
            ove.ovre_hooks.hadd_item scope EcTheory.noimport (Th_operator (x, newop))
  ...
  | Some { pl_desc = (prov, prmode); pl_loc = loc; } ->
    ...
    in
      ...
      let scope =
        match prmode with
        | `Alias ->
            ove.ovre_hooks.hadd_item scope import (Th_operator(x, newpr))

        | `Inline `Keep ->
            ove.ovre_hooks.hadd_item scope EcTheory.noimport (Th_operator(x, newpr))
...
and replay_ntd (ove : _ ovrenv) (subst, ops, proofs, scope) import (x, oont) =
  (* FIXME section : ~import:EcTheory.import0 make sens ? *)
  ...
  | None ->
    if EcPath.Sp.mem (xpath ove x) ove.ovre_ntclr then
      ...
      let scope = ove.ovre_hooks.hadd_item scope import (Th_operator (x, oont)) in
      (subst, ops, proofs, scope)
...
=
  ...
  (* FIXME section : EcTheory.import0 make sens ? *)
  ...
  | None ->
      ...
      (subst, ops, proofs, ove.ovre_hooks.hadd_item scope import (Th_modtype (x, modty)))
  ...
  | Some { pl_desc = (newname, mode) } ->
      ...
      let scope =
        ...
        then ove.ovre_hooks.hadd_item scope import (Th_modtype (name, newmt))
        else scope in
      (subst, ops, proofs, scope)

(* -------------------------------------------------------------------- *)
and replay_mod
  (ove : _ ovrenv) (subst, ops, proofs, scope) import tme =
  ...
  | None ->
      ...
      let tme = { tme_expr = me; tme_loca = (ove.ovre_local :> locality) } in
      (subst, ops, proofs,
       ove.ovre_hooks.hadd_item scope import (Th_module tme))
  ...
  | Some { pl_desc = (newname, mode) } ->
      ...
      let mp, (newme, _) = EcEnv.Mod.lookup (unloc newname) env in
      ...
      let scope =
        if   keep_of_mode mode
        then
          let tnewme = {tme_expr = newme; tme_loca = (ove.ovre_local:>locality)} in
          ove.ovre_hooks.hadd_item scope import (Th_module tnewme)
        else scope in
...
and replay_reduction
  ...
  import (rules : (EcPath.path * EcTheory.rule_option * EcTheory.rule option) list)
=
